from parser import Parser, Tree, CleanTree
...
class Form990Parser(Parser):
  ...
  def cleanTags(self, xTree: CleanTree) -> CleanTree:
    cleanedTree = {}
    for tag in xTree:
  ...
  def dict(self, depth: int = -1):
    trees = super().tree(depth, "{%s}" % self.namespace["irs"], self.groupsToSkip)
    return [trees[0], self.cleanTags(trees[1])]
